...
def compute_auroc(is_positive, is_rejected):
    ...
    for epsilon in range(840, 886):
        tp, tn, fp, fn = 0, 0, 0, 0
        for is_pos, is_rej in zip(is_positive, is_rejected[epsilon-840]):
...
def is_rejected_all(topk):
    ...
    for topk_val, topk_pos in zip(topk[0], topk[1]):
        is_rej = []
        for epsilon in range(840, 886):
            is_rej.append((topk_pos == 10 or topk_val < (epsilon / 1000.0)))
...
def save_rejected():
    sm = np.load("testing/sm.npy", allow_pickle=True)
    sm_topk = torch.topk(torch.from_numpy(sm), k=1, dim=1)
    is_rej_sm = is_rejected_all(sm_topk)
    np.save("testing/rejected/sm_rej_2.npy", is_rej_sm)
    ...
    for al_ta in range(3):
        ...
        np.save("testing/rejected/om_rej_2_a%d_t%d.npy" % (alpha[al_ta], tail[al_ta]), is_rej_om)
...
def grid_precision_recall():
    is_positive = np.concatenate((np.full(10000, True), np.full(10000, False)))
    sm_rej = np.load("testing/rejected/sm_rej_2.npy", allow_pickle=True)
    pr_om_all, re_om_all = [], []
    ...
    for al_ta in range(3):
        om_rej = np.load("testing/rejected/om_rej_2_a%d_t%d.npy" % (alpha[al_ta], tail[al_ta]), allow_pickle=True)
...
def grid_ca_eps():
    ...
    alpha = [3, 2, 1]
    tail = [3, 7, 2]
    for eps in range(840, 886):
        ca_om = np.zeros(3)
        ca_sm = classification_accuracy_epsilon(targets, sm, eps/1000.0)
        # counter = 0
        for al_ta in range(3):
            om = np.load("testing/om_a%d_t%d.npy" % (alpha[al_ta], tail[al_ta]), allow_pickle=True)
            ca_om[al_ta] = classification_accuracy_epsilon(targets, om, eps/1000.0)
            # counter += 1
            # print("%s " % (str(counter).zfill(3)), end="")
            # print("--- %s seconds ---" % (time.time() - start_time))
...
if __name__ == "__main__":
    ...
    print("ca_om*3, ca_sm")
    grid_ca_eps()
    print("precision_om*3, recall_om*3, precision_sm, recall_sm")
